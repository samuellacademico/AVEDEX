...
from src.avedex.utils import Cor, colorir, limpar_tela, mensagem_aviso, pausar


def selecionar_e_visualizar_imagem(aves):
    """Permite escolher uma ave e chama a visualização de imagem."""
    ave = escolher_ave(aves, "Escolha uma ave para visualizar a imagem")
    if ave is not None:
...
def selecionar_e_tocar_som(aves):
    ...
    ave = escolher_ave(aves, "Escolha uma ave para ouvir o som")
    if ave is not None:
        tocar_som(ave)
...
def executar():
    ...
    while True:
        ...
        opcao = input(colorir("Escolha uma opção: ", Cor.CIANO)).strip()
        ...
        if opcao == "1":
            ...
            limpar_tela()
            print(colorir("Obrigado por usar a AveDex!", Cor.VERDE))
            break
        else:
