"""Carregamento e validação do dataset da AveDex."""
...
def carregar_dataset(caminho=CAMINHO_DATASET):
    """Lê o arquivo JSON e devolve o dataset completo."""
    try:
...
def carregar_aves():
    """Carrega apenas a lista de aves do dataset."""
    dataset = carregar_dataset()
    return dataset.get("aves", [])


def obter_fontes_globais():
    """Carrega as fontes globais cadastradas no dataset."""
    dataset = carregar_dataset()
    return dataset.get("fontes_globais", {})


def validar_dataset(aves):
    """Verifica campos obrigatórios, mídia, duplicidade e tipos numéricos."""
    problemas = []
    ids_encontrados = set()
    ...
    for posicao, ave in enumerate(aves, start=1):
        ...
        if "comprimento_cm" in ave and not isinstance(
            ave.get("comprimento_cm"), (int, float)
        ):
        ...
        if "indice_conservacao" in ave and not isinstance(
            ave.get("indice_conservacao"), int
        ):
